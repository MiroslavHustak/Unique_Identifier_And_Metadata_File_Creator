...
let private whatIs(x: obj) =
    match x with
    | :? TaskResults as du ->
                            du  //aby nedoslo k nerizene chybe behem runtime
    | _                    -> 
...
let private getUniqueIdentifierCsvXlsxGoogle rowStart rowEnd startWithNumber reportProgress = //readGoogleSheetsData |> createUniqueIdentifier |> createCsvXlsxGoogle 
    ...
    let createUniqueIdentifier (dtGoogle: Data.DataTable) = //nevydedukoval
        ...
        let msg =  
            ...
            |> List.mapi
                (fun i item ->                 //Array.parallel tady nefungovalo s DataTable
                             ...
                             |> List.iteri 
                                 (fun j item ->
                                              dtGoogle.Rows.[i].[j] <- (myString dtGoogle.Rows.[i].[j]).Replace("–", "-").Replace(@"//", @"/").Replace(@"\\", @"\").Replace(",,", ",").Replace(";", ",")    
                             ...
                             let fnCisloKartonu = 
                                 ...
                                 | false -> 
                                          checkBoxFn (sprintf "%s%s" <| karTxb1 <| (myString dtGoogle.Rows.[i].[8])) karCkbxLeft
                                 | true  -> 
...
let getUniqueIdentifier rowStart rowEnd startWithNumber reportProgress =    
    let mainTxbString myDGSada = 
        let result = 
            ...
            | true  ->
                     myDGSada //getUniqueIdentifierCsvXlsxGoogle rowStart rowEnd startWithNumber reportProgress
            | false ->     
                     let myDG_Sada = 
                         {
                             errorDG = List.Empty
