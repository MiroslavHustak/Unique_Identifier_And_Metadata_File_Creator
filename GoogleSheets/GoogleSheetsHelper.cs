...
//zdrojovy kod pro DLL C# (neautorsky kod s vyjimkou jednoho velmi skaredeho hacku a kodu pro rozsireni poctu radku)
...
namespace GoogleSheetsHelper
{
    public class GoogleSheetsHelper
    {
        ...
        public List<ExpandoObject> GetDataFromSheet(GoogleSheetParameters googleSheetParameters)
        {
            ...
            //if (values.Count > 0)
            {
                foreach (var row in values)
                {
                    ...
                    {
                        if (row.Count > 0) //bez teto podminky vyhazuje kompilator chybu   
                        {
                            string uglyHack = "";
                           
                            try
                            {
                                uglyHack = row[columnCounter].ToString();
                            }
                            catch
                            {
                                uglyHack = "";  //Pokud posledni (a jen posledni, ty prazdne predtim nevadi) sloupec v Google tabulce obsahuje prazdne bunky, hodi to exception. Duvody neznam.
                            }
                            finally
                            {
                                expandoDict.Add(columnName, uglyHack);
                            }
                        }
                        else
                        {
        ...
        public void AddCells(GoogleSheetParameters googleSheetParameters, List<GoogleSheetRow> rows, int endIndex)
        {
            ...
            //userEnteredValue = ponecha formatovani, vymaze cely sheet, a jeste, coz neni v dokumentaci, zmeni default radky z 1000 na nevim kolik //UPDATE: asi nic nezmeni...
            ...
            DimensionRange dr1 = new DimensionRange
            {
                ...
                Dimension = "ROWS",
                StartIndex = 999,
                EndIndex = endIndex //napr. 6999 (6000 radku je 6999-999)
            };

            //prida napr. 6000 radku za defaultnich 1000 radku 
            ...
            requests.Requests.Add(request1); // pridany kod   //prida radky
            requests.Requests.Add(request2);   // pridany kod  //smaze sheet, ale ponecha formatovani           
